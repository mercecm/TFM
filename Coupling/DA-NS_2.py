...
mu = 0.005375
Re = (rho*D)/(eta*mu)
ext_f = red*(Lx*Lx*mu*c0*9)/(D*2*R*R*rhonp)
#p0 = 101325*(Lx*Lx*mu*mu)/(rho*D*D)
p0 = 1
...
while t<T:
	...
	solve(F3 == 0, p, bc_p, solver_parameters={"newton_solver":{"relative_tolerance":5e-9}}) #
	...
	if i%10 == 0:
		...
		plt.title ( 'Concentration, time %.5f' % ( t ), fontsize = 13 )
		plt.colorbar(c, orientation = 'horizontal', pad = 0.2)
		filename = ( 'couple_%.5f.png' % ( t ) )

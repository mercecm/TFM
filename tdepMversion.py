...
flux = project(mu*u_s.dx(0)+u_s,FunctionSpace(mesh, 'CG', 2))
flux_expr = project(mu*u_expr.dx(0)+u_expr,FunctionSpace(mesh, 'CG', 2))
flux_exact = interpolate (flux_expr,V)

#9. Plot and save output
fig = plt.figure ( )
ax = plt.subplot ( 111 )
plot (x[0], u_s, label = 'Computed' )
plot (x[0], u_exact, label = 'Exact' )
ax.legend ( )
ax.grid ( True )
